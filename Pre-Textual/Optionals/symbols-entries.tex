...
%%
%% Observações:
%% 1. Com subdivisões (\MakeSymbols* em ./utfpr-thesis.tex), os itens são
%%    ordenados pelo rótulo (label), mas conforme o tipo.
%% 2. Sem subdivisões (\MakeSymbols em ./utfpr-thesis.tex), os itens são
...
%% +==============+==============+===========+===========+
%%%% Para notação, o comando (termo) armazena um símbolo no comando \MrkSym
%%%% (\DottedCircle por padrão, no qual se aplica a notação durante a impressão
%%%% da mesma) com um segundo argumento (opcional: \nttn{Label}[MrkSym]).
